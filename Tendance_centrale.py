...
data = pd.read_csv('C:/Users/louis/Documents/LOUIS/INFO/SIGMA\PROJET 3A/data_roulage_km.csv')
                                                                    # Lecture du fichier .csv (nom ou adresse à modifier en fonction des données prises)
...
# dataP = np.sort(np.array(pd.read_csv('data_roulage_km.csv'))[:, 0]) # Lecture et transformation en tableau du fichier .csv (nom ou adresse à modifier en fonction des données prises)
...
# P = []                                                              # Future liste des 'upper estimates" des quantiles à 99% pour chaque loi
# P.append(dataP[round(0.99* len(dataP))])                            # Calcul et stockage de la valeur empirique du quantile à 99% (pour comparaison future)
...
# Cette section traite séparément les trois meilleures lois données par le Fit_Everything
# Si ces lois s'avèrent différentes des trois suivantes, il faudra remplacer les trois "Fit" suivants par d'autres (cf. documentation reliability)
# Il en va de même pour les légendes des graphes et des sorties suivantes
# (les données de la meilleure distribution peuvent déjà être obtenues avec le Fit_Everything)
