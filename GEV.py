...
On importe d'abord les données, puis on utilise la méthode des "block maxima" couplée à celle du "bootstrap" pour dientifier les paramètres 
de la GEV et son mode (valeur la plus probable du quantile). Ici, on étudie des données de roulage, mais en changeant l'adresse du fichier 
et les légendes des graphes, ce code peut être facilement adapté pour n'importe quel usage.
"""
...
p = 40001                                                           # Nombre de valeurs d'échantillonnage des GEV du bootstrap
start = 15E3                                                        # Début de l'échantillonnage des GEV
finish = 55E3                                                       # Fin de l'échantillonnage des GEV
...
ec = round(np.std(Q))
print('\n')
print('quantile à 99% : ', q, ' (écart-type = ', ec, ')')
...
mu = round(np.mean(Mu))
ec_m = round(np.std(Mu))
sigma = round(np.mean(Sigma))
ec_s = round(np.std(Sigma))
xi = round(np.mean(Xi), 3)
ec_x = round(np.std(Xi), 3)
print('\n')
print('--- Paramètres de la GEV ---')
print('mu = ', mu, ' (écart-type = ', ec_m, ')')
print('sigma = ', sigma, ' (écart-type = ', ec_s, ')')
print('xi = ', xi, ' (écart-type = ', ec_x, ')')